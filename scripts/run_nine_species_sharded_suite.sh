...
GPU_LIST="${4:?comma-separated GPU list required}"
...
echo "Running sharded suite: ${SUITE_NAME}"
echo "Config: ${CONFIG_PATH}"
echo "Results dir: ${RESULTS_DIR}"
echo "GPUs: ${GPU_LIST}"
echo "Spectra per shard: ${SPECTRA_PER_SHARD}"
...
for entry in "${SPECIES[@]}"; do
  ...
  cmd=(
    python "${PROJECT_ROOT}/scripts/run_species_sharded.py"
