#!/usr/bin/env python3
"""Case study for why InstaNovo reranking gains remain limited."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--ion-mode", default="y_heavy")
...
def norm(seq: str) -> str:
    return normalize_peptide(normalize_ptm_format(seq or "", model="instanovo"))
...
def main() -> None:
    ...
    for idx, (row, spectrum) in enumerate(zip(rows, spectra)):
        ...
        decoder_probs = softmax([float(beam["decoder_score"]) for beam in beams])
        ...
        beam_norms = [norm(seq) for seq in beam_sequences]
        baseline_top1 = row["top1"]
        baseline_norm = norm(baseline_top1)
    ...
    changed_wrong_rows = [r for r in analyzed if r["changed"] and (not r["chosen_correct"]) and (not r["baseline_correct"])]
    ...
    lines = [
        ...
        f"- baseline generation-failure rows (truth not in top-5 beam): `{len(generation_failure_rows)}`",
        "",
        "## Group Statistics",
    ]
