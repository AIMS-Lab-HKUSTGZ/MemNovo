...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Enable Spectrum Attention Enhancer to improve spectrum modality utilization",
)
@click.option(
    "--temperature",
    type=float,
    default=0.7,
    help="Temperature parameter for enhancer (default: 0.7, <1.0 = sharper attention)",
)
@click.option(
    "--alpha",
    type=float,
    default=0.3,
    help="Mixing ratio for enhancer (default: 0.3, 0=pure original, 1=pure enhanced)",
)
def sequence(
    peak_path: Tuple[str],
    model: Optional[str],
    ...
    with ModelRunner(
        ...
        # Apply Spectrum Attention Enhancer if requested
        enhancer = None
        if enable_enhancer:
            logger.info("=" * 60)
            logger.info("Enabling Spectrum Attention Enhancer")
            logger.info("  Temperature: %.2f", temperature)
            logger.info("  Alpha: %.2f", alpha)
            logger.info("  Target layers: [6, 7, 8]")
            logger.info("=" * 60)

            import sys
            sys.path.insert(0, '/ssdwork/wangzedong/ldx/instanovo/src')
            from spectrum_attention_enhancer import create_enhancer
            ...
            enhancer.patch_model(runner.model)
            logger.info("✓ Spectrum Attention Enhancer applied")

        results_path = output_path / f"{output_root_name}.mztab"
        runner.predict(peak_path, str(results_path), evaluate=evaluate)

        # Remove enhancer after prediction
        if enhancer is not None:
            enhancer.unpatch_model()
            logger.info("✓ Spectrum Attention Enhancer removed")
